...
This is a deliberate reframing of an earlier PSTAT 262DS project (Section
\ref{sec:relation}); the modeling code is reused, but the questions, the primary
pipeline, and the inferential machinery are recast for PSTAT 232.
...
The metadata feature set originally included \texttt{product\_average\_rating},
...
\paragraph{Models (reused ERM estimators).}
...
strong conflict miscalibration seen previously was largely a side-effect of
leakage.
...
Reframing a multimodal-disagreement study around computational statistics yields
sharper, more honest conclusions than a leaderboard framing. Empirical risk
...
revealing that most of the previously reported calibration pathology was a
leakage artifact. The accompanying scripts reproduce every figure and table
end-to-end from materialized data.

\section*{Relationship to the Prior PSTAT 262DS Project}
\label{sec:relation}
This project reuses the data pipeline, model code, and metric utilities of an
earlier PSTAT 262DS study (``When Modalities Disagree''). The reframing for
PSTAT 232 makes leakage control the \emph{primary} setting (previously an
appendix audit), foregrounds resampling-based inference, adds the
group-conditional calibration estimator, and replaces the leaderboard narrative
with a computational-statistics one. See \texttt{PSTAT232\_REFACTOR\_PLAN.md} for
a file-by-file account of what was reused, changed, and archived.
